...
default_path = 'preprocessed_data/'
...
def create_h5(data, filename = 'train_set'):
    data_ = np.array(data)
    h5f = h5py.File(default_path+filename+'.h5', 'w')
...
def create_json(filename, data, separators=('\n','\n')):
    with open(default_path+filename, 'w') as outfile:
        json.dump(data, outfile,separators=separators)

'''
runs the script for stadalone usage
'''
#get_data()
